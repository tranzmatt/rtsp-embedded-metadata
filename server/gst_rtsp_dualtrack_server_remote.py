...
def select_encoder():
    if has_element("nvv4l2h264enc"):
        print("[Encoder] Using NVIDIA Jetson hardware encoder (nvv4l2h264enc)")
        ...
        print("[Encoder] Using NVIDIA NVENC hardware encoder (nvh264enc)")
        return "nvh264enc preset=llhp"
    else:
        print("[Encoder] Using software encoder (x264enc)")
...
def main():
    ...
    if args.output:
        ...
        print(f"[Mode] Local RTSP server at rtsp://0.0.0.0:{args.port}{args.path}")
        loop = GLib.MainLoop()
