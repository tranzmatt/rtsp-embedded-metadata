#!/usr/bin/env python3
"""
YOLO RTSP Server — Frame-buffer synchronized version with drop policy.
Frames are held until their YOLO result is ready, but old frames are dropped
if the buffer grows too large to keep latency bounded.
"""
...
class YOLODetector:
    ...
    def queue_frame(self, fid, frame, ts, max_queue=6):
        """Queue a frame if space allows; else drop oldest."""
        with self.lock:
...
def main():
    ...
    ap.add_argument("--max-buffer", type=int, default=30,
                    help="Max buffered frames before dropping oldest")
    args = ap.parse_args()
    ...
    try:
        while True:
            ...
            # Drop oldest frames if buffer grows too large
            if len(frame_buffer) > args.max_buffer:
                oldest = min(frame_buffer)
                print(f"⚠ Dropping frame {oldest} (buffer overflow)")
