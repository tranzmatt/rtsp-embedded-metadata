#!/usr/bin/env python3
"""
RTSP Client — Option 2 with real-time YOLO overlay rendering.

Keeps a short buffer of frames for alignment. When metadata arrives,
the client finds the frame whose timestamp is closest to YOLO's analyzed_pts
and draws bounding boxes and labels on it.
"""
...
def main():
    ...
    ap.add_argument("--buffer-ms", type=int, default=800,
                    help="Client frame buffer window in ms")
    ...
    try:
        while True:
            ...
            # drop frames older than buffer_window
            while frame_buf and now - frame_buf[0][0] > buffer_window:
                frame_buf.popleft()

            m = meta.get()
            if not m or "yolo" not in m:
                # show most recent frame if nothing yet
                if not args.no_display:
                    cv2.imshow("RTSP Stream (Live)", frame)
                    if cv2.waitKey(1) & 0xFF == ord("q"):
                        break
                continue

            y = m["yolo"]
            target_pts = m["yolo"].get("analyzed_pts")
            if not target_pts:
                continue

            # find frame closest in time to YOLO timestamp
            if frame_buf:
                closest = min(frame_buf, key=lambda f: abs(f[0] - (m["_recv_time"] - (m["pts"] - target_pts))))
                disp = closest[1].copy()
                ...
                if time.time() - last_print > 1.0:
                    print(f"Frame {m.get('frame_id')} → {y.get('count',0)} detections "
                          f"({y.get('detection_time',0)*1000:.0f} ms)")
                    last_print = time.time()

                # show
                if not args.no_display:
                    cv2.imshow("RTSP Stream (Aligned + Overlay)", disp)
                    if cv2.waitKey(1) & 0xFF == ord("q"):
                        break
            else:
                if not args.no_display:
